 ...
 * Chainlink oracle.
 */
contract ETHPriceFeed is Ownable, CheckContract, IPriceFeed {
    ...
    function _chainlinkPriceChangeAboveMax(
         ...
         */
        uint256 percentDeviation = maxPrice.sub(minPrice).mul(1e18).div(
            maxPrice
        );
    ...
    function _bothOraclesSimilarPrice(
        ...
        uint256 minPrice = Math.min(scaledTellorPrice, scaledChainlinkPrice);
        uint256 maxPrice = Math.max(scaledTellorPrice, scaledChainlinkPrice);
        uint256 percentPriceDifference = maxPrice.sub(minPrice).mul(1e18).div(
            minPrice
        );
